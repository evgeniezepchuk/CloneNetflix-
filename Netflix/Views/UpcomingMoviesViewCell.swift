...
class  UpcomingMoviesViewCell: UITableViewCell {
    ...
    static let identifire = "UpcomingMoviesViewCell"
    var movies: [Movie] = []
    ...
    let collectionView = {
        ...
        cv.register(UICollectionViewCell.self, forCellWithReuseIdentifier: "UpcomingMoviesViewCell")
        return cv
    }()
...
extension UpcomingMoviesViewCell: UICollectionViewDelegate, UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        20
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "UpcomingMoviesViewCell", for: indexPath)
        let image = UIImageView(frame: CGRect(x: 0, y: 0, width: cell.bounds.width, height: cell.bounds.height))
        cell.backgroundView = image
        APICaller.shared.getUpcomingMovies(complition: { [weak self] result in
            switch result {
