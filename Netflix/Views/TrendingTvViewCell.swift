...
class  TrendingTvViewCell: UITableViewCell {
    
    static let identifire = "TrendingTvViewCell"
    var movies: [Movie] = []
    ...
    let collectionView = {
        ...
        cv.register(UICollectionViewCell.self, forCellWithReuseIdentifier: "TrendingTvViewCell")
        return cv
    }()
...
extension TrendingTvViewCell: UICollectionViewDelegate, UICollectionViewDataSource {
    
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "TrendingTvViewCell", for: indexPath)
        let image = UIImageView(frame: CGRect(x: 0, y: 0, width: cell.bounds.width, height: cell.bounds.height))
        cell.backgroundView = image
        APICaller.shared.getTrendingTv(complition: { [weak self] result in
            switch result {
